...
const DATASET_PAGE = 'https://data.taipei/dataset/detail?id=49802349-ad4e-4551-be78-668e247f4d16';
const DATASET_ID = '49802349-ad4e-4551-be78-668e247f4d16';
...
type ResourceIndex = { generatedAt: string; datasetPage: string; resources: Array<{ id: string; url: string; resourceName: string; fileName: string; downloadedAt: string; sha256: string; bytes: number; rowCount: number; encoding: 'utf8' }>; warnings: string[] };

async function main(): Promise<void> {
  const force = process.argv.includes('--force');
  await mkdir(rawDir, { recursive: true });
  const index = await readIndex();
  const ids = await discoverResourceIds().catch((error) => {
  ...
  for (const id of ids) {
    ...
    try {
      ...
      const fileName = `construction-stop-resume-work-${id}.csv`;
      ...
      const next = { id, url, resourceName: '臺北市停復工公開資訊', fileName, downloadedAt: new Date().toISOString(), sha256, bytes: bytes.length, rowCount: rows.length, encoding: 'utf8' as const };
      ...
      index.warnings.push(`${id}: ${String(error)}`);
    }
  }
...
async function discoverResourceIds(): Promise<string[]> {
  ...
  return [...new Set([...html.matchAll(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/gi)].map(([id]) => id).filter((id) => id !== DATASET_ID))];
}
...
function toCsv(rows: Array<Record<string, unknown>>): string {
  const headers = ['序號', '工程名稱', '事業單位名稱', '停工日期', '復工或復工審查日期', '停工範圍', '停工原因'];
